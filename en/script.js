...
const creatProjectList = (data) => {
  ...
  for (let index = 0; index < data.length; index += 1) {
    ...
    newButton.innerText = 'Click to expand/collapse';
    newLink.href = data[index].location;
    newLink.innerText = 'Project link';
    newLink.target = '_blank';
    newTextAbout.innerHTML = data[index].about;
    newTextTechnology.innerHTML = `Technologies used:<br></br> ${data[index].technology}`;
    newTextHowToUse.innerHTML = `How it works:<br></br>${data[index].howToUse}`;
    newTextResources.innerHTML = `Features:<br></br>${data[index].resources}`;
  }
